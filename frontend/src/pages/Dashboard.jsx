...
export default function Dashboard() {
  ...
  if (errorProducts)
    return <p className="text-red-500">Error: {errorProducts}</p>;
  ...
  const totalStockValue = products.reduce(
    (acc, item) => acc + item.quantity * parseFloat(item.cost_price),
    0
  );
  ...
  return (
    <div className="space-y-8">
      {/* Dashboard Header */}
      <div className="flex items-center justify-between bg-gray-50 p-6 rounded-2xl shadow-sm">
        <div className="flex items-center gap-5">
          <img
            src={logo}
            alt="Shepherd Logo"
            className="w-32 h-32 object-contain rounded-full bg-transparent"
          />
          <div>
            ...
            <p className="text-lg text-gray-500">
              Quick summary of your inventory system
            </p>
          </div>
        </div>
      </div>
