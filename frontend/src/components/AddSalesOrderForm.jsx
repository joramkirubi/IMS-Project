...
export default function AddSalesOrderForm({ onClose, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      <div className="bg-white w-96 p-6 rounded shadow-lg">
        <h3 className="text-lg font-semibold mb-3">Add Sales Order</h3>
        <form className="space-y-2" onSubmit={handleSubmit}>
          <input name="total_amount" onChange={handleChange} placeholder="Total amount" type="number" step="0.01" className="w-full border p-2 rounded" required />
          ...
          <select name="customer" onChange={handleChange} className="w-full border p-2 rounded" required>
            <option value="">Select customer</option>
            {customers.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>

          <select name="processed_by" onChange={handleChange} className="w-full border p-2 rounded">
